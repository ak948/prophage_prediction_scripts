...
with open('bacteriaprophagedefensecount.tsv', 'wt') as out_file:
    ...
    tsv_writer.writerow(["BiosampleID", "Prophage Defense System Count",
                         ...
                         "Prophage PHROGs Genes Count", "Prophage Non-PHROGs Genes Count", "Prophage % of PHROGs Genes",
                         "Prophage PHROGs Modification Genes Count", "Prophage PHROGs Resistance Genes Count",
                         "Prophage PHROGs Virulence Genes Count", "Prophage PHROGs Toxin Genes Count",
                         "Prophage PHROGs Immune Genes Count", "Prophage PHROGs Defense Genes Count",
                         "Prophage Modification and Toxin Genes Count", "Prophage Modification and Defense Genes Count",
                         "Prophage Resistance and Toxin Genes Count", "Prophage Virulence and Toxin Genes Count"])
...
for name in genome_name:
    ...
    phrogs_genes_count = 0
    non_phrogs_genes_count = 0
    modification_genes_count = 0
    resistance_genes_count = 0
    virulence_genes_count = 0
    toxin_genes_count = 0
    immune_genes_count = 0
    defense_genes_count = 0
    modification_and_toxin_genes_count = 0
    modification_and_defense_genes_count = 0
    resistance_and_toxin_genes_count = 0
    virulence_and_toxin_genes_count = 0

    with open('prophage_phage_defenses.tsv', 'r') as file:
        reader = csv.DictReader(file, delimiter='\t')
        for row in reader:
            if f'{name}_p' in row['Genome ID']:
                ...
                elif row['Defense Subtype'] == 'Avs_II': defense_system_15 += 1 #####################################
    ...
    with open('phrog_counts.tsv', 'r') as file:
        reader = csv.DictReader(file, delimiter='\t')
        for row in reader:
            if f'{name}_p' in row['Prophage Name']:
                phrogs_genes_count += int(row['PHROGs Genes Count'])
                non_phrogs_genes_count += int(row['Non-PHROGs Genes Count'])
                modification_genes_count += int(row['PHROGs Modification Genes Count'])
                resistance_genes_count += int(row['PHROGs Resistance Genes Count'])
                virulence_genes_count += int(row['PHROGs Virulence Genes Count'])
                toxin_genes_count += int(row['PHROGs Toxin Genes Count'])
                immune_genes_count += int(row['PHROGs Immune Genes Count'])
                defense_genes_count += int(row['PHROGs Defense Genes Count'])
                modification_and_toxin_genes_count += int(row['PHROGs Modification and Toxin Genes Count'])
                modification_and_defense_genes_count += int(row['PHROGs Modification and Defense Genes Count'])
                resistance_and_toxin_genes_count += int(row['PHROGs Resistance and Toxin Genes Count'])
                virulence_and_toxin_genes_count += int(row['PHROGs Virulence and Toxin Genes Count'])

    total_genes_count = int(phrogs_genes_count) + int(non_phrogs_genes_count)
    phrogs_genes_percent = (int(phrogs_genes_count) / int(total_genes_count)) * 100
    ...
    with open('bacteriaprophagedefensecount.tsv', 'a') as out_file:
        ...
        tsv_writer.writerow([f'{name}', f'{defense_system_count}',
                             ...
                             f'{anti_defense_system_16}', f'{phrogs_genes_count}', f'{non_phrogs_genes_count}',
                             f'{phrogs_genes_percent}', f'{modification_genes_count}', f'{resistance_genes_count}',
                             f'{virulence_genes_count}', f'{toxin_genes_count}', f'{immune_genes_count}',
                             f'{defense_genes_count}', f'{modification_and_toxin_genes_count}',
                             f'{modification_and_defense_genes_count}', f'{resistance_and_toxin_genes_count}',
                             f'{virulence_and_toxin_genes_count}'])
