    ...
    with open('Output/results.tsv', 'wt') as out_file:
        ...
        tsv_writer.writerow(
            ['Biosampleid', 'Genbank Info Species', 'Collection Date', 'Collection Year', 'Collection Location',
             'Collection Country', 'Isolation Source', 'Genome Length', 'Genome Defense System Count', 'Genome AMR Count',
             'Number of Prophages', 'Average Prophage Length', 'Minimum Prophage Length', 'Maximum Prophage Length',
...
if not os.path.isfile('Output/genome_amr_genes.tsv'):
    with open('Output/genome_amr_genes.tsv', 'wt') as out_file:
        tsv_writer = csv.writer(out_file, delimiter='\t')
        tsv_writer.writerow(['AMR Gene ID', 'ORF ID', 'Source Sequence', 'Cut-Off', '% Identity', 'Drug Class',
...
if not os.path.isfile('Output/genome_phage_defenses.tsv'):
    with open('Output/genome_phage_defenses.tsv', 'wt') as out_file:
        tsv_writer = csv.writer(out_file, delimiter='\t')
        tsv_writer.writerow(['ID', 'Genome ID', 'System ID', 'Defense Type', 'Defense Subtype', 'System Start', 'System End',
                             'Proteins in System', 'Genes in System', 'Profiles in System'])
...
for id in biosampleid:
    ...
    os.mkdir(f'{id}_genome')
    os.chdir(f'{id}_genome')
    with open(f'../{id}/info.txt', 'r') as file:
        line = file.readline().strip()  # Read the first line
        accession1 = line.split('\t')[1]
    retry_limit = 100  # Maximum number of retries
    retry_count = 0
    while retry_count < retry_limit:
        try:
            # Download Enterococcus genome
            Entrez.email = "ak948@le.ac.uk"
            # Get ID for BioSampleID
            handle = Entrez.esearch(db="assembly", term=accession1)
            ...
            # Rename fasta file to match BioSampleID
            os.rename(f'{label}.fna.gz', f'{id}.fna.gz')
        ...
        print("Genome not downloaded")

    if os.path.isfile(f'{id}.fna.gz'):
        with gzip.open(f'{id}.fna.gz', 'rb') as f_in:
            with open(f'{id}.fna', 'wb') as f_out:
                shutil.copyfileobj(f_in, f_out)
        os.remove(f'{id}.fna.gz')
        ...
        with open(f'{id}_defense_finder_systems.tsv', 'r') as file:
            ...
            for row in reader:
                ...
                with open('../Output/genome_phage_defenses.tsv', 'a') as out_file:
                    tsv_writer = csv.writer(out_file, delimiter='\t')
        ...
        subprocess.run(f'rm *.prt* *defense_finder*', shell=True)

        #Run gtdbtk on enterococcus genome to find its species
       # subprocess.run(f'gtdbtk classify_wf --genome_dir ./ --out_dir ./gtdbtk --skip_ani_screen', shell=True)
        ...
        with open(f'{id}_rgi.txt', 'r') as file:
            ...
            for row in reader:
                ...
                if float(percentage_length) >= 75 and float(percentage_coverage) >= 90:
                    with open('../Output/genome_amr_genes.tsv', 'a') as out_file:
                        tsv_writer = csv.writer(out_file, delimiter='\t')
        ...
        genome_amr_genes = "N/A"
        genome_length = "N/A"

    os.chdir('..')
    shutil.rmtree(f'{id}_genome')
    ...
    for filename in os.listdir():
        if filename.startswith(f'{id}_p'):
            ...
            subprocess.run(f'rm {id}_p{file_num}_rgi.txt output.tsv', shell=True)
    ...
    with open('results.tsv', 'a') as out_file:
        ...
        tsv_writer.writerow([f'{id}', f'{final}', f'{isolation_date}', f'{year}', f'{isolation_location}', f'{country}',
                             f'{isolation_source}', f'{genome_length}', f'{genome_defense_systems}', f'{genome_amr_genes}',
...
if args.skip_figures is False:
    ...
    virulence_gene_counts = data['Total number of virulence genes in prophages']
    ...
    pylab.title("Number of virulence genes per genome")
    ...
    pylab.title('Sum of Number of Prophage AMRs by Species')
    pylab.xticks(rotation=20, ha='right')
    pylab.subplots_adjust(bottom=0.30)
    pylab.rc('xtick', labelsize=5)
    pylab.rc('ytick', labelsize=8)
    ...
    pylab.ylabel('Count')
    pylab.title('Number of genomes for each Isolation Country')
    pylab.xticks(rotation=20, ha='right')
    pylab.subplots_adjust(bottom=0.30)
    pylab.rc('xtick', labelsize=5)
    pylab.rc('ytick', labelsize=8)
    ...
    pylab.ylabel('Count')
    pylab.title('Number of genomes for each Year of Isolation')
    pylab.xticks(rotation=20, ha='right')
    pylab.subplots_adjust(bottom=0.30)
    pylab.rc('xtick', labelsize=5)
    pylab.rc('ytick', labelsize=8)
    ...
    for bar in bar_plot:
        ...
        label = "{:.2f}".format(y_val)  # Format y_val to two decimal places
        pylab.text(x_val, y_val, label, ha='center', va='bottom')
    pylab.savefig("averageamrperisolationyear")
    plt.close()